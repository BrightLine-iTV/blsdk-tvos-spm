...
#import <BLAdFramework/BLCoreEvents.h>
#import "UDKCore.h"
...
-(void)initFramework:(NSString *)configurationURL withAnalytics:(NSString*)analyticsURL withDelegate:(id)delegate disableAnalytics:(BOOL)disableAnalytics;

// UDK
-(void)initMetalDevice:(id<MTLDevice>)device layer:(CAMetalLayer *)layer;
...
-(void)showAd:(UIView *)viewController withDefinition:(NSString*)adDefinition withPlayerView:(UIView*)playerView;
...
/*
    ...
    * params
            ...
            - withAdPods: represents the transpose JSON data from the VAST data
                            schema for loading the ad
 
 
 */
-(void)setAds:(UIView *)viewController withAdPods:(NSArray*)adPods withPlayerView:(UIView*)playerView;
...
-(void)didUpdateState:(NSString*)state;

-(void)didUpdateAdPosition:(float)currentTime;

-(void)updateClientPlayerPosition:(float)currentTime;
